...
function showLoading(on) {
  ...
  if (on) {
    ...
    el.innerHTML     = '<div class="spinner"></div><p>Generating AI Report — this may take 15–30 seconds…</p>';
  } else {
...
function buildCharts(v) {
  ...
  chartBP = new Chart(document.getElementById('chartBP'), {
    ...
    data: {
      ...
      datasets: [{ label:'mmHg', data:[sys, dia, 120, 80],
        backgroundColor: [
  ...
  chartSugar = new Chart(document.getElementById('chartSugar'), {
    type: 'doughnut',
    data: { datasets:[{ data:[sug||0, Math.max(350-(sug||0),0)],
  ...
  chartBMI = new Chart(document.getElementById('chartBMI'), {
    type: 'doughnut',
    data: { datasets:[{ data:[bmi||0, Math.max(45-(bmi||0),0)],
  ...
  chartOther = new Chart(document.getElementById('chartOther'), {
    ...
    data: {
      ...
      datasets: [{ label:'Value', data:[temp, pulse, 37, 72],
        backgroundColor: [
